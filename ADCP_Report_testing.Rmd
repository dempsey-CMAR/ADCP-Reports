...
station <- "Cornwallis NE"    # station and depl_date can be converted to params
depl_date <- "2020-10-22"
...
# This now works well; copy to real report template.
...
# Make the buffer smaller for stations with small leases, and bigger for stations further out to sea. (Move this section over to the real report template?)

zoom_in_stations <- c("Brier Island", "Brier Island E", "Superlease")
# Review these, of course
zoom_out_stations <- c("Brandy Ledge", "Center Bay", "Blandford")
# Brandy Ledge may need even more zooming out
...
**Notes:**
  ...
  - Current speed intervals are right-inclusive.
...
# The depth needs to say "Not Recorded" whenever it is blank, but this is best done in the adcp_write_report_table function.
...
```{r error=FALSE, message=FALSE, warning=TRUE, results="asis"}
...
for(i in 24:28){
  ...
  ints.i <- adcp_count_obs(dat.i$SPEED, label_sep = "\n")
  
  breaks.i <- c(ints.i$lower - 0.000001, max(ints.i$upper) + 0.000001) # NOT a mistake that using max(upper)
  ...
  # NOTE: subtracting and adding 0.000001 solves the warning message issue and the "range" issue, but it is not a perfect solution. It would be better if we could change the package.

  # generate  current rose
  k <- k + 1

  adcp_plot_current_rose(dat.i, breaks = breaks.i, speed_cols = speed_cols)
  # there is a warning message associated with the windRose function:
  # Warning message:
  # In openair::windRose(dat, ws = "SPEED", wd = "DIRECTION", breaks = breaks,  :
  # Some values are below minimum break.
  
  # Relevant section of code from the windRose function:
